...
const Blog = ({ blog, handleBookmarks, handleTotalTime }) => {
    // console.log(blog)
    return (
        <>
            <div className='space-y-4 mb-5'>
                ...
                <div className='flex justify-between items-center'>
                    ...
                    <p className='text-[#11111199] flex gap-2 items-center'>{`${blog.reading_time} min read `} <button onClick={() => handleBookmarks(blog)}><BiBookmark></BiBookmark></button></p>
                </div>
                <h1 className='font-bold text-4xl'>{blog.title}</h1>
                <p className='text-[#11111199]'>{`#${blog.tags[0]} #${blog.tags[1]} #${blog.tags[2]}`}</p>
                <button onClick={() => handleTotalTime(blog.id, blog.reading_time)} className='text-[#6047EC] border-b-2 border-[#6047EC]'>Mark as read</button>
            </div>
        </>
    );
};
